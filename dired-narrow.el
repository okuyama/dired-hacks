      ...
      (while (dired-hacks-next-file)
        (if (funcall dired-narrow-filter-function filter)
            (progn
              ...
              (when (and (featurep 'dired-subtree)
                         (< 0 (dired-subtree--get-depth (dired-subtree--get-ov))))
                (save-excursion
                  (let ((dirname (directory-file-name (file-name-directory (directory-file-name (dired-get-filename))))))
                    (--dotimes (dired-subtree--get-depth (dired-subtree--get-ov))
                      (dired-utils-goto-line dirname)
                      (when (get-text-property (line-beginning-position) :dired-narrow)
                        (remove-text-properties (1- (line-beginning-position)) (line-end-position) '(:dired-narrow nil))
                        (remove-text-properties (1- (line-beginning-position)) (line-end-position) '(invisible nil))
                        (setq visible-files-cnt (1+ visible-files-cnt)))
                      (setq dirname (directory-file-name (file-name-directory (directory-file-name dirname)))))))))
          (put-text-property (1- (line-beginning-position)) (line-end-position) :dired-narrow t)
          (put-text-property (1- (line-beginning-position)) (line-end-position) 'invisible :dired-narrow))))
